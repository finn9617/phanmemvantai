...
<section class="content-header">
    <div class="row">
        <div class="col-md-12 titleDieuXe"> SỬA THÔNG TIN BẢO DƯỠNG  </div>
    </div>
    <div class="row">
                    ...
                    <form action="/maintenance/create" method="post" name="itemForm" id="acountForm">
                        <div class="row">
                            <div class="form-group col-md-6 "> 
                                <label for="email">Loại xe hoặc loại rơ mooc (*):</label><label style="color: red; font-size: 13px"><i id="error-loaixe"></i></label>
                                <select class="select2" style="width:100%" name = "selLoaixe" id = "selLoaixe" data-placeholder="-- Chọn loại xe --">
                                    <option></option>
                                </select>
                            </div> 
                            <div class="form-group col-md-6 "> 
                                <label for="email">Số xe hoặc số rơ mooc (*):</label><label style="color: red; font-size: 13px"><i id="error-soxe"></i></label>
                        ...
                        <div class="row">
                            ...
                            <input type="text" class="form-control" name="txtSoPhieu" id="txtSoPhieu" placeholder="Nhập số phiếu bảo dưỡng..." value="{{ $maintenance->votes }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group date">
                                    ...
                                    <input class="form-control" placeholder="Từ ngày" name="date" id="date" value="{{ $maintenance->expiration_date }}" type="date">
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="" class="control-label">Ghi chú </label>
                            <textarea class="form-control" rows="5" placeholder="Nhập ghi chú nếu cần" name="txtGhichu" id="txtGhichu">{{ $maintenance->note }}</textarea>
                        </div>
                        <div class="row">
...
<script>
  ...
  $(document).ready(function (){
    // let preURL = document.referrer +'?top='+{{$maintenance->maintenance_id}};
    //                         alert(preURL);
        $('#btnOk').click(function(e){
            ...
            $.ajax({
                data:data,
                url: "{{ url('/maintenance/detail/'.$maintenance->maintenance_id)}}",
                type: "POST",
                headers: {
                ...
                success: function(data){
                    ...
                    if(data.success){
                        if ('referrer' in document) {
                           ...
                           // alert(prePage);
                            let topPage = {{$maintenance->maintenance_id}};
                            let typePage = 'maintenance';
    ...
    @if(isset($maintenance->car_id))
        $("#selSoxe").val({{ $maintenance->car_id }});
    @endif

    @if(isset($maintenance->car_type_id))
        $("#selLoaixe").val({{ $maintenance->car_type_id }});
    @endif
</script>
@endsection