...
<section class="content-header">
      <div class="row">
        <div class="col-md-12 titleDieuXe">DANH MỤC {{ App\TitleList::ListTitle('maintenance') }}</div>
      </div>
      <!-- ./ title -->
</section>
<section class="content">
  <div class="box box-primary">
    <div class="box-header container-fluid">
     <form  action = "/maintenance/search" name="maintenanceform" id="maintenanceform" method="GET">
        <meta name="csrf-token" content="{{ csrf_token() }}">
            <div class="col-sm-3"></div>
                <select class="select2 col-md-3  " name = "selmaintenance" id = "selmaintenance" data-placeholder="-- Chọn số xe --">
                        <option value=""></option>
                    @for($i=0 ; $i < count($maintenance_car); $i++)
                        <option @if(isset($maintenance_id)) @if($maintenance_id == $maintenance_car[$i]->car_id) selected @endif @endif value="{{ $maintenance_car[$i]->car_id }}">{{ $maintenance_car[$i] ->car_num }}</option>
                    @endfor
                </select>

              <button type="submit" class="btn btn-success" style="margin-left:1%" form="maintenanceform">Tìm kiếm</button>
              <a class="btn btn-success" style="margin-left:1%" href="/maintenance">Tất cả</a>
    </form>
  </div>
  <div class="row">
        <div class="container-fluid">
            <div class="col-md-1">
                <div class="form-group">
                    <a href="/maintenance/create" class="btn btn-success pull-left"><i class="fa fa-plus"></i>&nbsp;&nbsp;Thêm mới</a>
                </div>
            </div>
            <div class="col-md-2">
                <a class="btn btn-info pull-left" data-toggle="modal" data-target="#modal-default" style="margin-left: 3px; background: #3c8dbc;">
                        <?php $n = 0; ?>
                        @foreach((isset($maintenance_bd)) ? $maintenance_bd : $maintenance  as $bd)
                            <?php
     ...
     <div class="modal fade in" id="modal-default" style="display: none;">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                                ...
                                <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            ...
                                            : activate to sort column ascending">Số phiếu bảo dưỡng
                                                        </th>
                                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Ghi chú
                                                                : activate to sort column ascending">Ngày hết hạn bảo dưỡng
                                                        </th>
                                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Ghi chú
                ...
                <a class="btn btn-success" style="float:right;" target="framename" href="{{ route('printM') }}"> IN DANH SÁCH </a>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
        ...
        <table id="maintenance" class="table table-bordered  dataTable table-hover" role="grid" aria-describedby="example2_info">
          <thead style="background-color: #3C8DBC; color: #FFFFFF">
            <tr role="row">
              ...
              <th class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" >CHI TIẾT BẢO DƯỠNG</th>
          ...
          <tbody>
            ...
            <tr <?php if( intval($diff->format("%a")) <= 7 || $diff->format("%R") === '-' ) echo 'style="background:#1296f345;color:blue;"'; ?> >
              ...
              <td>
                ...
                <td>{{ $bd->note}}</td>
            <td><a style="text-align:center;" href="{{ url('/maintenance/itemDetail/'.$bd->maintenance_id)}}">Xem chi tiết</a></td>
                <td style="width: 80px">
                    <a class="edit" title="Sửa" href="/maintenance/detail/{{$bd->maintenance_id}}"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;&nbsp;&nbsp;
                    <a class="delete" href="#" type="button" onclick="btnDelete({{$bd->maintenance_id}})" title="Xóa" ><i class="glyphicon glyphicon-trash"></i></a>&nbsp;&nbsp;&nbsp;
                </td>
            </tr>
            @endforeach
          </tbody>
