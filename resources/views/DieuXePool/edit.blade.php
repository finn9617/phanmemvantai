...
<!--     <ol class="breadcrumb">
      ...
      <!-- title -->
      <div class="row">
        <div class="col-md-12 titleDieuXe">THÊM THÔNG TIN ĐIỀU XE ĐÃ CHỈNH SỬA</div>
      </div>
      <!-- ./ title -->
      <!-- back page -->
      <div class="row">
        <div class="col-md-12 prePage">
          <a href="/operatingPool" class="">
            <span class="glyphicon glyphicon-step-backward">
              <span class="prePage">DANH SÁCH ĐIỀU XE ĐÃ CHỈNH SỬA </span>
            </span>
          </a>
        </div>
      </div>
...
<script>
  ...
  function calculateToolNumModeComplete(){
    if(resData['operating'][0]['status'] == 2){// mode complete{
...
function getLastestGoods(){
    ...
    //  ajax
    $.ajax('{{url("/operatingPool/getLastestGoods")}}', {
      type: 'POST',  
      data: frmInfo,
      dataType:"json",
      async: false,
    ...
    //  ajax
    $.ajax('{{url("/operatingPool/getLastestCarInfoByPlace")}}', {
      type: 'POST',  
      data: frmInfo,
      dataType:"json",
      async: false,
 ...
 //FUNCTION GET DRIVER OR ASISTANT DRIVER FOR OPERATING
    ...
    function getDriverInfoForOperating(type = 0){
      ...
      frmInfo['type'] = type;
    //  ajax
    $.ajax('{{url("/operatingPool/getDriverInfoForOperating")}}', {
      type: 'POST',  
      data: frmInfo,
      dataType:"json",
      async: false,
...
if(operating['before_receipt_note'] || operating['after_receipt_note']){
  ...
  // ============== Submit form ==========================
    ...
    // =======================
    var routeName = '{{url("/postEditOperatingPool")}}';
    ...
    $.ajax(routeName, {
      ...
      success: function (result) {
       ...
       if(result.success)
       {
        ...
        .then((value) => {
          window.location.href = "/operatingPool";
        });
        setTimeout(function(){ window.location.href = "/operatingPool"; }, 3000);
      }
      if(result.error)
      {
        swal("Thất bại", "Có lỗi trong quá trình xử lý", "error");
