...
<section class="content-header">
      <div class="row">
        <div class="col-md-12 titleDieuXe">DANH MỤC {{ App\TitleList::ListTitle('oil') }}</div>
      </div>
      <!-- ./ title -->
</section>
<section class="content">
  <div class="box box-primary">
    <div class="box-header container-fluid">
     <form  action = "/oil/search" name="oilform" id="oilform" method="GET">
        <meta name="csrf-token" content="{{ csrf_token() }}">
            <div class="col-sm-3"></div>
                <select class="select2 col-md-3  " name = "seloil" id = "seloil" data-placeholder="-- Chọn số xe --">
                        <option value=""></option>
                    @for($i=0 ; $i < count($oil_car); $i++)
                        <option @if(isset($oil_id)) @if($oil_id == $oil_car[$i]->car_id) selected @endif @endif value="{{ $oil_car[$i]->car_id }}">{{ $oil_car[$i] ->car_num }}</option>
                    @endfor
                </select>

              <button type="submit" class="btn btn-success" style="margin-left:1%" form="oilform">Tìm kiếm</button>
              <a class="btn btn-success" style="margin-left:1%" href="/oil">Tất cả</a>
    </form>
  </div>
  <div class="row">
        <div class="container-fluid">
            <div class="col-md-1">
                <div class="form-group">
                    <a href="/oil/create" class="btn btn-success pull-left"><i class="fa fa-plus"></i>&nbsp;&nbsp;Thêm mới</a>
                </div>
            </div>
            <div class="col-md-2">
                <a class="btn btn-info pull-left" data-toggle="modal" data-target="#modal-default" style="margin-left: 3px; background: #3c8dbc;">
                        <?php $n = 0; ?>
                        @foreach((isset($oil_o)) ? $oil_o : $oil  as $o)
                        @endforeach
  ...
  <div class="box-body">
      <div class="table-responsive">
        <table id="oil" class="table table-bordered  dataTable table-hover" role="grid" aria-describedby="example2_info">
          <thead style="background-color: #3C8DBC; color: #FFFFFF">
            <tr role="row">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{ $o->note_oil}}</td> 
              <td style="width: 80px">
                <a class="edit" title="Sửa" href="/oil/detail/{{$o->oil_id}}"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;&nbsp;&nbsp;
                <a class="delete" href="#" type="button" onclick="btnDelete({{$o->oil_id}})" title="Xóa" ><i class="glyphicon glyphicon-trash"></i></a>&nbsp;&nbsp;&nbsp;
              </td>
            </tr>
            @endforeach
          </tbody>
...
<script>
  function btnDelete($id){
    swal({
			title: "Xóa báo thay nhớt xe",
            text: "Bạn có chắc chắn muốn xóa Báo Thay Nhớt này không ?",
            icon: "warning",
			buttons: {
				confirm: 'Có',
				cancel: 'Hủy'
