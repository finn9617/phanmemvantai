...
<section class="content-header">
    <div class="row">
        <div class="col-md-12 titleDieuXe"> SỨA THÔNG TIN BÁO THAY NHỚT  </div>
    </div>
    <div class="row">
        <div class="col-md-12 prePage">
        <a href="{{ route('showO') }}" onclick="back()" class="" id="back">
                <span class="glyphicon glyphicon-step-backward">
                    <span class="prePage">DANH SÁCH BÁO THAY NHỚT </span>
                </span>
            </a>
        </div>
    </div>
                ...
                <div class="box-body">
                    <form action="/baoduong/create" method="post" name="itemForm" id="acountForm">
                        ...
                        <div class="row">
                            ...
                            <input type="text" class="form-control" name="txtSocu" id="txtSocu" placeholder="Nhập số km công tơ củ..." value="{{ $oil->num_old }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="somoi">Số km công tơ mới (*):</label><label style="color: red; font-size: 13px"><i id="error-date"></i></label>
                                <input type="text" class="form-control" name="txtSomoi" id="txtSomoi" placeholder="Nhập số km công tơ mới..." value="{{ $oil->num_new }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="sothay">Số km cần thay nhớt (*):</label><label style="color: red; font-size: 13px"><i id="error-date"></i></label>
                                <input type="text" class="form-control" name="txtSothay" id="txtSothay" placeholder="Nhập số km cần thay..." value="{{ $oil->num_change }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="" class="control-label">Ghi chú </label>
                        <textarea class="form-control" rows="5" placeholder="Nhập ghi chú nếu cần" name="txtGhichu" id="txtGhichu">{{$oil->note_oil}}</textarea>
                        </div>
                        <div class="row">
...
<script>
  ...
  $(document).ready(function (){
    ...
    @if(isset($oil->car_id))
        $("#selSoxe").val({{ $oil->car_id }});
    @endif

    @if(isset($oil->car_type_id))
        $("#selLoaixe").val({{ $oil->car_type_id }});
    @endif
</script>
@endsection