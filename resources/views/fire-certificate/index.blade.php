...
<section class="content-header">
      <div class="row">
        <div class="col-md-12 titleDieuXe">DANH MỤC {{ App\TitleList::ListTitle('fire-certificate') }}</div>
      </div>
      <!-- ./ title -->
</section>
<section class="content">
  <div class="box box-primary">
    <div class="box-header container-fluid">
     <form  action = "/fire-certificate/search" name="fireCertificateform" id="fireCertificateform" method="GET">
        <meta name="csrf-token" content="{{ csrf_token() }}">
            <div class="col-sm-3"></div>
                <select class="select2 col-md-3  " name = "selfireCertificate" id = "selfireCertificate" data-placeholder="-- Chọn số xe --">
                        <option value=""></option>
                    @for($i=0 ; $i < count($fireCertificate_car); $i++)
                        <option @if(isset($fireCertificate_id)) @if($fireCertificate_id == $fireCertificate_car[$i]->car_id) selected @endif @endif value="{{ $fireCertificate_car[$i]->car_id }}">{{ $fireCertificate_car[$i] ->car_num }}</option>
                    @endfor
                </select>

              <button type="submit" class="btn btn-success" style="margin-left:1%" form="fireCertificateform">Tìm kiếm</button>
              <a class="btn btn-success" style="margin-left:1%" href="/fire-certificate">Tất cả</a>
    </form>
  </div>
  <div class="row">
        <div class="container-fluid">
            <div class="col-md-1">
                <div class="form-group">
                    <a href="/fire-certificate/create" class="btn btn-success pull-left"><i class="fa fa-plus"></i>&nbsp;&nbsp;Thêm mới</a>
                </div>
            </div>
            <div class="col-md-2">
                <a class="btn btn-info pull-left" data-toggle="modal" data-target="#modal-default" style="margin-left: 3px; background: #3c8dbc;">
                        <?php $n = 0; ?>
                        @foreach( (isset($fireCertificate_hh)) ? $fireCertificate_hh : $fireCertificate as $cn)
                            <?php
     ...
     <div class="modal fade in" id="modal-default" style="display: none;">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <a class="btn btn-success" style="float:right;" target="framename" href="{{ route('printF') }}"> IN DANH SÁCH </a>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
          ...
          <tbody>
            ...
            <tr <?php if( intval($diff->format("%a")) <= 7 || $diff->format("%R") === '-' ) echo 'style="background:#1296f345;color:blue;"'; ?> >
              ...
              <td style="width: 80px">
                ...
                <a class="delete" href="#" type="button" onclick="btnDelete({{$cn->fire_certificate_id}})" title="Xóa" ><i class="glyphicon glyphicon-trash"></i></a>&nbsp;&nbsp;&nbsp;
              </td>
            </tr>
            @endforeach
          </tbody>
		...
		.then((willDelete) => {
			if (willDelete) {
				$.ajax({
					...
					{
						if(data.success){
                            window.location = "{{ url('/fire-certificate')}}"
						}
						if(data.errors){
							swal({
