...
<section class="content-header">
    <div class="row">
        <div class="col-md-12 titleDieuXe"> SỬA THÔNG TIN BẢO HIỂM  </div>
    </div>
    <div class="row">
        <div class="col-md-12 prePage">
        <a href="{{ route('showInsu') }}" onclick="back()" class="" id="back">
                <span class="glyphicon glyphicon-step-backward">
                    <span class="prePage">DANH SÁCH BẢO HIỂM </span>
                </span>
            </a>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-warning col-md-11">
                <div class="box-body">
                    <form action="/insurance/create" method="post" name="itemForm" id="acountForm">
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="email">Số phiếu bảo hiểm (*):</label><label style="color: red; font-size: 13px"><i id="error-sophieu"></i></label>
                            <input type="number" class="form-control" name="txtSoPhieu" id="txtSoPhieu" placeholder="Nhập số phiếu bảo hiểm..." value="{{ $insurance->votes }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group date">
                                    ...
                                    <input class="form-control" placeholder="Từ ngày" name="date" id="date" value="{{ $insurance->expiration_date }}" type="date">
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="" class="control-label">Ghi chú </label>
                            <textarea class="form-control" rows="5" placeholder="Nhập ghi chú nếu cần" name="txtGhichu" id="txtGhichu">{{ $insurance->note }}</textarea>
                        </div>
                        <div class="row">
...
<script>
  ...
  $(document).ready(function (){
        $('#btnOk').click(function(e){
            ...
            $.ajax({
                data:data,
                url: "{{ url('/insurance/detail/'.$insurance->insurance_id)}}",
                type: "POST",
                headers: {
                ...
                success: function(data){
                    ...
                    if(data.success){
                        if ('referrer' in document) {
                           ...
                           // alert(prePage);
                            let topPage = {{$insurance->insurance_id}};
                            let typePage = 'insurance';
    ...
    @if(isset($insurance->car_id))
        $("#selSoxe").val({{ $insurance->car_id }});
    @endif

    @if(isset($insurance->car_type_id))
        $("#selLoaixe").val({{ $insurance->car_type_id }});
    @endif
</script>
@endsection