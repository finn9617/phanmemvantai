...
<section class="content-header">
      <div class="row">
        <div class="col-md-12 titleDieuXe">DANH MỤC {{ App\TitleList::ListTitle('verify') }}</div>
      </div>
      <!-- ./ title -->
</section>
<section class="content">
  <div class="box box-primary">
    <div class="box-header container-fluid">
     <form  action = "/verify/search" name="verifyform" id="verifyform" method="GET">
        <meta name="csrf-token" content="{{ csrf_token() }}">
            <div class="col-sm-3"></div>
                <select class="select2 col-md-3  " name = "selverify" id = "selverify" data-placeholder="-- Chọn số xe --">
                        <option value=""></option>
                    @for($i=0 ; $i < count($verify_car); $i++)
                        <option @if(isset($verify_id)) @if($verify_id == $verify_car[$i]->car_id) selected @endif @endif value="{{ $verify_car[$i]->car_id }}">{{ $verify_car[$i] ->car_num }}</option>
                    @endfor
                </select>

              <button type="submit" class="btn btn-success" style="margin-left:1%" form="verifyform">Tìm kiếm</button>
              <a class="btn btn-success" style="margin-left:1%" href="/verify">Tất cả</a>
    </form>
  </div>
  <div class="row">
        <div class="container-fluid">
            <div class="col-md-1">
                <div class="form-group">
                    <a href="/verify/create" class="btn btn-success pull-left"><i class="fa fa-plus"></i>&nbsp;&nbsp;Thêm mới</a>
                </div>
            </div>
            <div class="col-md-2">
                <a class="btn btn-info pull-left" data-toggle="modal" data-target="#modal-default" style="margin-left: 3px; background: #3c8dbc;">
                        <?php $n = 0; ?>
                        @foreach( (isset($verify_hh)) ? $verify_hh : $verify as $kd)
                            <?php
     ...
     <div class="modal fade in" id="modal-default" style="display: none;">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                                ...
                                <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            ...
                                            : activate to sort column ascending">Số phiếu KD
                                                        </th>
                                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Ghi chú
                ...
                <a class="btn btn-success" style="float:right;" target="framename" href="{{ route('printV') }}"> IN DANH SÁCH </a>
                </div>
            </div>
            <!-- /.modal-content -->
        </div>
        ...
        <table id="verify" class="table table-bordered  dataTable table-hover" role="grid" aria-describedby="example2_info">
          <thead style="background-color: #3C8DBC; color: #FFFFFF">
            <tr role="row">
              ...
              <th class="sorting_asc" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" >SỐ PHIẾU KD</th>
          ...
          <tbody>
            ...
            <tr <?php if( intval($diff->format("%a")) <= 7 || $diff->format("%R") === '-' ) echo 'style="background:#1296f345;color:blue;"'; ?> >
              ...
              <td style="width: 80px">
                ...
                <a class="delete" href="#" type="button" onclick="btnDelete({{$kd->verify_id}})" title="Xóa" ><i class="glyphicon glyphicon-trash"></i></a>&nbsp;&nbsp;&nbsp;
              </td>
            </tr>
            @endforeach
          </tbody>
