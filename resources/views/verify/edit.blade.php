...
<section class="content-header">
    ...
    <div class="row">
        ...
        <a href="{{ route('showV') }}" onclick="back()" class="" id="back">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-warning col-md-11">
                <div class="box-body">
                    <form action="/verify/create" method="post" name="itemForm" id="acountForm">
                        <div class="row">
                        ...
                        <div class="row">
                            ...
                            <input type="number" class="form-control" name="txtSoPhieu" id="txtSoPhieu" placeholder="Nhập số phiếu bảo hiểm..." value="{{ $verify->votes }}">
                            </div>
                        </div>
                        <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <div class="input-group date">
                                        ...
                                        <input class="form-control" placeholder="Từ ngày" name="date" id="date" value="{{ $verify->register_date }}" type="date">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <div class="input-group date">
                                        <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                        <input class="form-control" placeholder="Từ ngày" name="date1" id="date1" value="{{ $verify->expiration_date }}" type="date">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="" class="control-label">Ghi chú </label>
                            <textarea class="form-control" rows="5" placeholder="Nhập ghi chú nếu cần" name="txtGhichu" id="txtGhichu">{{ $verify->note }}</textarea>
                            </div>
                        <div class="row">
  ...
  $(document).ready(function (){
        $('#btnOk').click(function(e){
            ...
            if(date1 < date2 || start == '' || end == '')
            {
                 $.ajax({
                data:data,
                url: "{{ url('/verify/detail/'.$verify->verify_id)}}",
                type: "POST",
                headers: {
    ...
    @if(isset($verify->car_id))
        $("#selSoxe").val({{ $verify->car_id }});
    @endif

    @if(isset($verify->car_type_id))
        $("#selLoaixe").val({{ $verify->car_type_id }});
    @endif
</script>
@endsection