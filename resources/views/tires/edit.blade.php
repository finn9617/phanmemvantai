...
<section class="content-header">
    <div class="row">
        <div class="col-md-12 titleDieuXe"> SỨA THÔNG TIN BÁO THAY VỎ  </div>
    </div>
    <div class="row">
        <div class="col-md-12 prePage">
        <a href="{{ route('showO') }}" onclick="back()" class="" id="back">
                <span class="glyphicon glyphicon-step-backward">
                    <span class="prePage">DANH SÁCH BÁO THAY VỎ</span>
                </span>
            </a>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-warning col-md-11">
                <div class="box-body">
                    <form action="/baoduong/create" method="post" name="itemForm" id="acountForm">
                        <div class="row">
                            <div class="form-group col-md-6 "> 
                                <label for="loaixe">Loại xe (*):</label><label style="color: red; font-size: 13px"><i id="error-loaixe"></i></label>
                                <select class="select2" style="width:100%" name = "selLoaixe" id = "selLoaixe" data-placeholder="-- Chọn loại xe --">
                                    <option></option>
                                </select>
                            </div> 
                            <div class="form-group col-md-6 "> 
                                <label for="soxe">Số xe (*):</label><label style="color: red; font-size: 13px"><i id="error-soxe"></i></label>
                        ...
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="socu">Số km công tơ củ (*):</label><label style="color: red; font-size: 13px"><i id="error-sophieu"></i></label>
                            <input type="text" class="form-control" name="txtSocu" id="txtSocu" placeholder="Nhập số km công tơ củ..." value="{{ $tires->num_old }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="somoi">Số km công tơ mới (*):</label><label style="color: red; font-size: 13px"><i id="error-date"></i></label>
                                <input type="text" class="form-control" name="txtSomoi" id="txtSomoi" placeholder="Nhập số km công tơ mới..." value="{{ $tires->num_new }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="sothay">Số km cần thay nhớt (*):</label><label style="color: red; font-size: 13px"><i id="error-date"></i></label>
                                <input type="text" class="form-control" name="txtSothay" id="txtSothay" placeholder="Nhập số km cần thay..." value="{{ $tires->num_change }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="" class="control-label">Ghi chú </label>
                        <textarea class="form-control" rows="5" placeholder="Nhập ghi chú nếu cần" name="txtGhichu" id="txtGhichu">{{$tires->note_tires}}</textarea>
                        </div>
                        <div class="row">
...
<script>
  ...
  $(document).ready(function (){
        $('#btnOk').click(function(e){
            ...
            data.append('txtSocu', $("#txtSocu").val());
            data.append('txtSomoi', $("#txtSomoi").val());
            data.append('txtSothay', $("#txtSothay").val());
            data.append('txtGhichu', $("#txtGhichu").val());


            $.ajax({
                data:data,
                url: "{{ url('/tires/detail/'.$tires->tires_id)}}",
                type: "POST",
                headers: {
    ...
    @if(isset($tires->car_id))
        $("#selSoxe").val({{ $tires->car_id }});
    @endif

    @if(isset($tires->car_type_id))
        $("#selLoaixe").val({{ $tires->car_type_id }});
    @endif
</script>
@endsection