...
class curatorController extends Controller
{
    ...
    {
        $search_tdd = DB::table('tbl_user')->where('user_type', 15)->orderby('full_name', 'ASC')->orderby('nick_name', 'ASC')->get();
        $search_tdx = DB::table('tbl_user')->where('user_type', 15)->orderby('nick_name', 'ASC')->get();
        ...
        $assistant = DB::table('tbl_user')->select('tbl_user.*')->where('tbl_user.user_type', '=', 15);
        ...
        //     return view('Curator.index', compact('assistant', 'search_tdd', 'search_tdx'));
        // } else {
        //     $assistant = DB::table('tbl_user')->where('user_type', 13)->orderby('nick_name', 'ASC')->get();
    ...
    public function AddCuratorGet(Request $request)
    {
        return view('Curator.create');
    }
    
    public function AddCuratorPost(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'tendixe.unique' => 'Tên viết tắt đã tồn tại',
        ...
        if ($validator->passes()) {
            if ($request->tendixe) {
                ...
                $tendixe = DB::select("SELECT COUNT(*) as cnt FROM tbl_user WHERE tbl_user.user_type = 15 AND CONVERT(tbl_user.nick_name,BINARY) = CONVERT('$request->tendixe',BINARY) AND tbl_user.work_status = 0");
                //var_dump($tendixe);
                // exit;
                if ($tendixe[0]->cnt > 0) {
                    $error = (object) ['tendixe' => ['Tên viết tắt không được trùng']];
                    return response()->json(['errors' => $error]);
                }
                if($request->check!=1){
                    //dd($request->check);exit;
                    $checkTen = DB::select("SELECT COUNT(*) as cnt FROM tbl_user WHERE tbl_user.user_type = 15 AND CONVERT(tbl_user.nick_name,BINARY) = CONVERT('$request->tendixe',BINARY) AND tbl_user.work_status = 1");
                    if ($checkTen[0]->cnt > 0){
                        $error = (object) ['checkten' => ['Tên viết tắt trùng với người đã nghỉ, bạn vẫn muốn tạo?']];
    ...
    public function DeleteCuratorPost(Request $request)
    {
    ...
    public function EditCuratorGet(Request $request, $id)
    {
        $user = DB::table('tbl_user')->where('user_id', $id)->first();

        return view('Curator.edit', compact('user'));
    }

    public function EditCuratorPost(Request $request, $id)
    {
        //dd($request->sdt);
        $validator = Validator::make($request->all(), [
            'hoten' => 'required',
        ...
        if ($validator->passes()) {
            if ($request->tendixe) {
                $tendixe = DB::select("SELECT COUNT(*) as cnt FROM tbl_user WHERE tbl_user.user_type = 15 AND CONVERT(tbl_user.nick_name,BINARY) = CONVERT('$request->tendixe',BINARY) AND tbl_user.user_id NOT IN ($id) AND tbl_user.work_status = 0");
